...
- **HU-1.3** Filtrar por tipo, emisor, año. Ordenar por recientes/antiguas/A-Z/Z-A. Filtros combinables
- **HU-1.4** Tarjetas con: imagen (principal), nombre, emisor, año, tipo de credencial, status (Activa/Vencida/Revocada visualmente distinguible), tags
- **HU-1.5** Organización por tipo (Títulos primero) y dentro por emisor. Scroll horizontal por sección, vertical entre secciones
...
- **HU-2.2** Filtrar por origen (Tec/Externo), emisor, año. Ordenar
- **HU-2.3** Tarjetas CLR con mosaico de hasta 4 imágenes + indicador "+N", nombre, emisor, año, status, tags
- **HU-2.4** Organización por origen: secciones "Tec" y "Externo"
- **HU-2.5** Importar CLRs (mismo botón que credenciales, auto-detecta tipo)
- **HU-2.6** Estado vacío de CLRs con explicación y botón importar

### E3 - Detalle de Credencial (página completa, NO modal)
- **HU-3.1** Imagen principal con zoom a pantalla completa
...
- **HU-3.6** Compartir: link público, LinkedIn, descargar JSON, descargar PNG baked

### E4 - Detalle de Título (Blockcerts)
- **HU-4.1** Imagen principal con zoom
- **HU-4.2** Sección Título: nombre, subtítulo, descripción, criterios, fechas, status, tags, alineamientos
- **HU-4.3** Sección Receptor: nombre completo
- **HU-4.4** Sección Emisor con líneas de firma (nombre, cargo, imagen firma)
- **HU-4.5** Verificación Blockchain: TX ID, tipo blockchain, hash, llave pública, fecha prueba, link explorador
- **HU-4.6** Descargar: JSON Blockcerts, XML SEP, PDF oficial
- **HU-4.7** Compartir: link público, LinkedIn

### E5 - Detalle de CLR
- **HU-5.1** Credenciales contenidas como sección principal (clickeables a su detalle)
- **HU-5.2** Info del CLR: nombre, descripción, status, fechas, indicador parcial/completo, evidencias, términos
- **HU-5.3** Sección Receptor
- **HU-5.4** Sección Emisor
- **HU-5.5** Compartir: link público, descargar JSON, LinkedIn
...
- **HU-7.2** Info del título público
- **HU-7.3** Receptor y emisor con firmas
- **HU-7.4** Verificación blockchain pública

### E8 - Página Pública de CLR
- **HU-8.1** Lista de credenciales contenidas
- **HU-8.2** Info del CLR público
- **HU-8.3** Receptor público
- **HU-8.4** Emisor público

### E9 - Verificador Público
- **HU-9.1** Verificar credencial por ID o JSON. Resultado: válida/inválida/revocada con desglose por área
